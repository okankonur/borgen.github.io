import { darkVariantPath } from "./svg-dark-variant.js";
...
//
// An SVG image gets two extra layers. A `div.figure-scroll` wrapper, because
// the diagrams are drawn on a fixed, wide canvas and on a phone they scale
// down far past the point where their labels are readable — the wrapper is
// what css/site.css hangs the sideways-scroll rule on. And a <picture> that
// points dark-mode viewports at the diagram's generated `.dark.svg` twin,
// because Safari will not apply the page's colour scheme to an SVG loaded
// through <img>; see lib/svg-dark-variant.js. Photographs get neither — they
// lose nothing by shrinking, and they have no colour scheme to match.
function wrapLoneImagesInFigure(md) {
  md.core.ruler.push("tufte_figure", (state) => {
    ...
    for (let i = 0; i < tokens.length - 2; i++) {
      ...
      const isDiagram = /\.svg(?:[?#]|$)/i.test(src);
      const dark = md.utils.escapeHtml(darkVariantPath(src));
      const media = isDiagram
        ? `<div class="figure-scroll"><picture>` +
...
//
// (a) tufte.css styles `div.table-wrapper` (overflow-x: auto, sans-serif
//     figures) but a markdown table renders as a bare <table>, so a wide one
//     pushes the whole page sideways instead of scrolling inside its own box.
//     Every table gets wrapped in that div.
//
// (b) Markdown gives no way to size a column, and markdown-it-attrs does not
//     work on tables (it appends a stray row instead). So a line of the form
//
//         {cols: 16% 20% 64%}
//
//     placed directly above a table is consumed and rendered as a <colgroup>,
//     with `table-layout: fixed` (via .cols-fixed) so the widths are honoured
//     exactly rather than treated as hints. Any CSS length works; there must
//     be one value per column.
function wrapTablesInWrapper(md) {
  const COLS_DIRECTIVE = /^\{cols:\s*([^}]+)\}$/;

  // Hoist a `{cols: ...}` paragraph onto the table token that follows it, and
  // drop the paragraph so it never renders as text.
  md.core.ruler.push("tufte_table_cols", (state) => {
    const tokens = state.tokens;
    for (let i = 0; i < tokens.length - 3; i++) {
      if (tokens[i].type !== "paragraph_open") continue;
      if (tokens[i + 2].type !== "paragraph_close") continue;
      if (tokens[i + 3].type !== "table_open") continue;

      const match = COLS_DIRECTIVE.exec(tokens[i + 1].content.trim());
      if (!match) continue;

      tokens[i + 3].meta = { ...tokens[i + 3].meta, cols: match[1].trim().split(/\s+/) };
      tokens.splice(i, 3);
      i--;
    }
  });

  // renderToken is kept so any future attrs still land on the <table> itself.
  md.renderer.rules.table_open = (tokens, idx, options, env, slf) => {
    const cols = tokens[idx].meta && tokens[idx].meta.cols;
    if (cols) {
      const existing = tokens[idx].attrGet("class");
      tokens[idx].attrSet("class", existing ? `${existing} cols-fixed` : "cols-fixed");
    }
    const colgroup = cols
      ? `<colgroup>${cols.map((w) => `<col style="width:${w}"/>`).join("")}</colgroup>\n`
      : "";
    return '<div class="table-wrapper">\n' + slf.renderToken(tokens, idx, options) + colgroup;
  };
