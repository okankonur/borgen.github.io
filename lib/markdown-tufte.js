...
//
// An SVG image additionally gets a `div.figure-scroll` around it. The SVG
// diagrams are drawn on a fixed, wide canvas, so on a phone they scale down
// far past the point where their labels are readable; the wrapper is what
// css/site.css hangs the sideways-scroll rule on. Photographs are left
// unwrapped — they lose nothing by shrinking.
function wrapLoneImagesInFigure(md) {
  md.core.ruler.push("tufte_figure", (state) => {
    ...
    for (let i = 0; i < tokens.length - 2; i++) {
      ...
      const isDiagram = /\.svg(?:[?#]|$)/i.test(img.attrGet("src") || "");
      const media = isDiagram ? `<div class="figure-scroll">${imgHtml}</div>` : imgHtml;
